...
public class DisapproveEmployeeCommand implements Command {

	private static final Logger logger = Logger.getLogger(DisapproveEmployeeCommand.class);

	private final String COMMAND_GO_TO_SUCCESS_PAGE = "?command=go_to_success_page_command";
	private final String MESSAGE_GO_TO_SUCCESS_PAGE = "&message=success_employee_disapproved";
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (userLogin != null) {
			...
			try {
				employeeStatusService.disapproveEmployee(employeeId);
