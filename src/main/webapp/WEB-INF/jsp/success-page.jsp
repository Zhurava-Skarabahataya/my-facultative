...
<fmt:message bundle="${loc}" key="success_apply_message"
	var="success_apply_message" />
<fmt:message bundle="${loc}" key="success_remove_apply_message"
	var="success_remove_apply_message" />
<fmt:message bundle="${loc}" key="approve_student_message"
	var="approve_student_message" />
<fmt:message bundle="${loc}" key="return_to_run_course_page"
	var="return_to_run_course_page" />
<fmt:message bundle="${loc}" key="return_to_user_courses_page"
	var="return_to_user_courses_page" />
<fmt:message bundle="${loc}" key="return_to_user_page"
	var="return_to_user_page" />
<fmt:message bundle="${loc}" key="success_created_course_message"
	var="success_created_course_message" />
<fmt:message bundle="${loc}" key="success_registration"
	var="success_registration" />
<fmt:message bundle="${loc}" key="success_authorization"
	var="success_authorization" />
<fmt:message bundle="${loc}" key="success_student_approved"
	var="success_student_approved" />
<fmt:message bundle="${loc}" key="success_employee_disapproved"
	var="success_employee_disapproved" />
<fmt:message bundle="${loc}" key="success_employee_approved"
	var="success_employee_approved" />
<fmt:message bundle="${loc}" key="go_to_user_courses_page"
	var="go_to_user_courses_page" />
...
<body>
	...
	<div class="inscription">
		<c:choose>
			...
			<c:when test="${requestScope.message =='success_student_approved'}">
				...
				<form action="Controller" method="post">
					<input type="hidden" name="command" value="go_to_user_courses_page" />
					<input type="submit" value="${go_to_user_courses_page}" />
				</form>
			...
			<c:when
				test="${requestScope.message =='success_employee_disapproved'}">
			...
			<c:when test="${requestScope.message=='student_approved'}">
				...
				<form action="Controller" method="post">
					...
					<input type="hidden" name="run_course_id"
						value="${requestScope.run_course_id}" /> <input type="submit"
						value="${return_to_run_course_page}" />
				</form>
			</c:when>
	...
	<img align="middle" width="300px" alt="" src="image/success.jpg">

	<form action="Controller" method="post">
		<input type="hidden" name="command" value="go_to_welcome_page" /> <input
			type="submit" value="${to_main_page}" /><br />
	</form>

	<form action="Controller" method="post">
		<input type="hidden" name="command" value="go_to_user_page" /> <input
			type="submit" value="${return_to_user_page}" />
	</form>
