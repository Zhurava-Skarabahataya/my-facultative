...
<fmt:setBundle
	basename="localization.user_courses_page.user_courses_page" var="loc"
	scope="session" />
...
<fmt:message bundle="${loc}" key="amount_of_students_on_course"
	var="amount_of_students_on_course" />
<fmt:message bundle="${loc}" key="students_limit_on_course"
	var="students_limit_on_course" />
...
<fmt:message bundle="${loc}" key="more_about_course"
	var="more_about_course" />
<fmt:message bundle="${loc}" key="no_courses_yet" var="no_courses_yet" />
<fmt:message bundle="${loc}" key="view_available_run_courses"
	var="view_available_run_courses" />
<fmt:message bundle="${loc}" key="course_ended" var="course_ended" />
<fmt:message bundle="${loc}" key="ended_courses" var="ended_courses" />
<fmt:message bundle="${loc}" key="course_cancelled"
	var="course_cancelled" />
<fmt:message bundle="${loc}" key="course_recruting"
	var="course_recruting" />
			...
			<table>
				...
				<c:forEach var="course" items="${sessionScope.bean.currentCourses}">
					<tr>
						...
						<td><c:choose>
								<c:when test="${course.currentState == 1}">
									<c:out value="${course_canselled}" />
								</c:when>
								<c:when test="${course.currentState== 2}">
									<c:out value="${course_ended}" />
								</c:when>
								<c:when test="${course.currentState== 3}">
									<c:out value="${course_recruting}" />
								</c:when>
								<c:when test="${course.currentState== 4}">
									<c:out value="${course_running}" />
								</c:when>
						...
						<td>
							<form action="Controller" method="post">
								<input type="hidden" name="command"
									value="go_to_run_course_page" /> <input type="hidden"
									name="run_course_id" value="${course.runCourseId}" /><input
									type="submit" value="${more_about_course}" />
							</form>
						</td>
					</tr>
				</c:forEach>
