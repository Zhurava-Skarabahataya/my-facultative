...
<title><c:out value="${user_page_title}"/></title>
...
<fmt:message bundle="${loc}" key="button.my_rating" var="my_rating" />
<fmt:message bundle="${loc}" key="button.create_course"
	var="create_course" />
<fmt:message bundle="${loc}" key="field.first_name" var="first_name" />
<fmt:message bundle="${loc}" key="field.second_name" var="second_name" />
<fmt:message bundle="${loc}" key="field.patronymic" var="patronymic" />
<fmt:message bundle="${loc}" key="field.role" var="role" />
<fmt:message bundle="${loc}" key="field.faculty" var="faculty" />
<fmt:message bundle="${loc}" key="field.adress" var="adress" />
<fmt:message bundle="${loc}" key="field.date_of_birth"
	var="date_of_birth" />
<fmt:message bundle="${loc}" key="field.tel_number" var="tel_number" />
<fmt:message bundle="${loc}" key="button.edit" var="edit" />
<fmt:message bundle="${loc}" key="button.go_to_staff_page"
	var="go_to_staff_page" />
<fmt:message bundle="${loc}" key="button.go_to_students_page"
	var="go_to_students_page" />
<fmt:message bundle="${loc}" key="log_out" var="log_out" />
<fmt:message bundle="${loc}" key="user_page_title" var="user_page_title" />
...
<body>
	...
	<div style="display: inline;">
		...
		<c:if test="${sessionScope.bean.userRoleId >2 }">
			...
			<form style="display: inline; float: center" action="Controller"
				method="post">
				<input type="hidden" name="command" value="go_to_students_page" />
				<input type="submit" value="${go_to_students_page}" /><br />
			</form>
	...
	<div class="user_photo" align="center">
		...
		<img src="${sessionScope.bean.userPhotoLink}"
			onerror="this.src='D:/Java/facultative-project/user_photos/default.jpg'"
			alt="${upload_photo}" height=200 width=200> <br>
		...
		<div align="center">
			<form action="Controller" method="post" enctype="multipart/form-data">
				<input type="hidden" name="command" value="upload_user_photo" /> <input
					...
					type="submit" value="${key_upload_photo}"
					name="${key_upload_photo}" />
			</form>
		</div>
	</div>
